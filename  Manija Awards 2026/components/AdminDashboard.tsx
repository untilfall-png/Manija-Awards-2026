...
export function AdminDashboard() {
  ...
  useEffect(() => {
    if (isAuthenticated) checkVotingStatus()
  }, [isAuthenticated])
  ...
  const handleToggleVoting = async () => {
    ...
    if (success) setVotingEnabledState(!votingEnabled)
  }
    ...
    const loadStats = async () => {
      try {
        ...
        setStats({
          ...
          participationRate: uniqueVoters > 0 ? Math.round((votes.length / uniqueVoters) * 100) : 0,
        })
      } catch (error) {
    ...
    return (
      <div className="text-center py-12">
        <div className="inline-flex items-center gap-2">
          ...
          <span className="text-neon-pink">Cargando estadísticas...</span>
        </div>
      </div>
    )
  }
