...
Set-Location 'C:\Kimi\ Manija Awards 2026\public\renders'

# Rename back for editing
Rename-Item 'index.html' 'manija-conclusion.html' -Force

# Read the file
$content = Get-Content 'manija-conclusion.html' -Raw

# The __timelines should be per-frame, not a single object
# Hyperframes expects window.__timelines = { [compositionId]: { duration, frames, layers } }
# But the simpler approach: just make sure HTML has correct metadata

# Let's try with minimal valid composition
$newContent = @'
<!DOCTYPE html>
...
window.__timelines["manija-conclusion"]={duration:300};
window.__hf={duration:300,seek:function(f){r(f)}};
const d={g:[{c:"Mejor Director",n:"Director A",v:342,i:"🎬",cr:"#ff00ff"},{c:"Mejor Actor",n:"Actor X",v:289,i:"🏆",cr:"#00ffff"},{c:"Mejor Actriz",n:"Actriz M",v:356,i:"👑",cr:"#ffff00"},{c:"Mejor Película",n:"Accion Total",v:423,i:"⭐",cr:"#00ff88"}],tv:156,tv2:1245,ch:{n:"Accion Total",v:423,c:"Mejor Película"}};
function r(f){
var a=document.getElementById("a");if(!a)return;
var y=(f*2.5)%1920,x=(f*1.8)%1920;
var h='<div style="width:1920px;height:1080px;background:radial-gradient(ellipse at 50% 40%,#1a0a2e 0%,#000 70%);position:relative;overflow:hidden;">';
...
Write-Host '✅ index.html created for hyperframes'
